...
#            metaData = {}
    ...
    def segGaussianTests(arr):
         ...
         for sid in np.arange(nseg):
            ...
            segFeatures[sid] = GaussianTests(segarr)
        return segFeatures
    
    metaData['SegGaussianTests'] = segGaussianTests(timeSeries) # this is not useful (i think), because we have already normalized the data at dm=0
...
# FEATURE IDEAS:
# count the number of points in the time series that exceed X * std centered on the median, positive and negative (two features), run on dedisp timeseries
